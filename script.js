...
(() => {
	...
	const CONFIG = {
		...
		spawnEveryMs: 200 ,   // interval between drops
		...
		soundBurnSrc: 'sounds/burn.wav',
		soundPopVolume: 1,
		soundPianoVolume: 0.8,
		soundBurnVolume: 0.8,
		soundPianoLoop: true,
		soundPianoDelayMs: 600, // when to start piano after blast
		soundPopPreMs: 600,     // play pop this many ms before the blast
		soundBurnBeforePopMs: 5100, // play burn this many ms before pop
	...
	function initAudio() {
		try {
			...
			sndBurn = new Audio(CONFIG.soundBurnSrc);
			sndBurn.preload = 'auto';
			sndBurn.volume = CONFIG.soundBurnVolume ?? 0.8;
		} catch {}
	}
	...
	function finalizeSelection() {
		...
		const onClick = () => {
			...
			// Pre-play burn, then pop slightly before the blast
			const lead = CONFIG.soundPopPreMs ?? 100;
			const preDelay = Math.max(0, dur - Math.max(0, lead));
			const burnGap = Math.max(0, CONFIG.soundBurnBeforePopMs ?? 150);
			const burnDelay = Math.max(0, preDelay - burnGap);
			setTimeout(() => { try { if (sndBurn) { sndBurn.currentTime = 0; sndBurn.play().catch(() => {}); } } catch {} }, burnDelay);
			setTimeout(() => {
				try {
